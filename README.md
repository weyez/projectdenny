...
Теперь страница использует локальный `proxy_server.py`, поэтому сайт загружается внутри окна браузера через `/proxy?url=...` и работает заметно лучше, чем прямой `iframe`.
