...
Локально страница использует `proxy_server.py`, поэтому сайт загружается внутри окна браузера через `/proxy?url=...` и работает заметно лучше, чем прямой `iframe`.

На GitHub Pages Python-сервер запустить нельзя, поэтому автоматически включается веб-proxy (`r.jina.ai`) для режима «Открыть внутри».
